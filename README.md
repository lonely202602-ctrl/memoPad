...
| 文件 | 说明 |
|------|------|
| [memoPad_0.5.0_x64-setup.exe · 1.4MB](https://github.com/lonely202602-ctrl/memoPad/releases/download/v0.5.0/memoPad_0.5.0_x64-setup.exe) | 安装版（推荐），按当前用户安装，无需管理员 |
| [memoPad_0.5.0_x64_portable.exe · 4.5MB](https://github.com/lonely202602-ctrl/memoPad/releases/download/v0.5.0/memoPad_0.5.0_x64_portable.exe) | 便携版，单文件双击即用，删除即卸载 |
...
> 这是 SmartScreen 对**未签名**新发布程序的常规提醒（本项目暂未购买代码签名证书），并非病毒误报之外的问题。
> 请点击 **「更多信息」→「仍要运行」** 即可正常安装；本应用完全本地运行、不联网上传任何数据。
...
- 安装版：Windows「设置 → 应用」中卸载，会自动清理开机自启项
  - 默认安装到独立目录 `%LOCALAPPDATA%\memoPad`；安装时若自选其他目录，程序也会**自动装入所选目录下的 `memoPad` 专属子文件夹**（如选 `D:\Tools` → `D:\Tools\memoPad`），不会与目录里的其他文件混放
  - 卸载**只删除程序自己的文件**（主程序 + 卸载器）及快捷方式，你放在安装目录里的其他文件不会被删除
